...
// ตรวจสอบว่าผู้ใช้ได้เข้าสู่ระบบแล้วหรือยัง และมีสิทธิ์เป็น major หรือไม่
...
require '../db_connect.php'; // ปรับพาธเนื่องจากอยู่ในโฟลเดอร์ faculty
...
// ในไฟล์ที่ผู้ใช้เข้าสู่ระบบสำเร็จ หรือในส่วนต้นของไฟล์ add_personal.php
...
if (isset($_SESSION['major_id'])) {
    $stmt = $pdo->prepare("SELECT major_name FROM major WHERE major_id = :major_id");
    $stmt->execute([':major_id' => $_SESSION['major_id']]);
    $major = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($major) {
        $_SESSION['major_name'] = $major['major_name'];
    } else {
        // จัดการกรณีที่ไม่พบข้อมูลสาขาวิชา
        $_SESSION['major_name'] = '';
    }
}
...
<body>
        ...
        <form id="personalForm" method="POST" action="add_personal.php" enctype="multipart/form-data">
            ...
            <div id="section3" class="form-section">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="faculty_name" name="faculty_name" value="<?= htmlspecialchars($_SESSION['faculty_name']) ?>">
                </div>

                <div class="mb-3">
                    <label for="major_name" class="form-label">สาขา</label>
                    <input type="text" class="form-control" id="major_name" name="major_name" value="<?= htmlspecialchars($_SESSION['major_name']) ?>" readonly>
                </div>
    ...
    <script>
        function nextSection() {
            ...
            } else if (document.getElementById('section2').classList.contains('active')) {
                currentSection = 'section2';
            }

            // รีเซ็ตข้อความข้อผิดพลาด
            document.querySelectorAll('#' + currentSection + ' .error-message').forEach(function(el) {
                el.textContent = '';
            });

            // ตรวจสอบฟิลด์ที่จำเป็น
            document.querySelectorAll('#' + currentSection + ' [required]').forEach(function(input) {
                if (!input.value.trim()) {
                    isValid = false;
            ...
            if (isValid) {
                ...
                // เลื่อนหน้าจอไปที่ฟิลด์แรกที่มีข้อผิดพลาด
                document.querySelector('#' + currentSection + ' .error-message').scrollIntoView({
                    behavior: 'smooth'
                });
            }
        }
